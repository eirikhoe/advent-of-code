    ...
    def min_path(self, endpoint):
        ...
        while len(queue) > 0:
            ...
            for candidate in [
                ...
                if (
                    (min(candidate) >= 0) and (candidate not in distance)
                ) and self.open(candidate):
...
def main():
    ...
    print("Part 1")
    print(
        f"It takes a minimum of {maze.min_path(point)} steps to reach {str(point)[1:-1]}"
    )
    print()
    ...
    max_dist = 50
    print(
        f"You can reach {maze.reachable_locs(max_dist)} locations in at most {max_dist} steps"
    )
