    ...
    while n_keys < key_lim:
        ...
        for j in range(1,len(hashed_string)):
            if hashed_string[j] == hashed_string[j-1]:
                ...
                counter = 1
            stops = (j == (len(hashed_string)-1)) or (hashed_string[j+1] != hashed_string[j])
...
def main():
    ...
    print("Part 2")
    print(f"With hash-stretching activated index {gen_keys(salt,True)} generates the 64th one-time pad key")
