...
def find_delay(data):
    reg = re.compile("Disc #(\d+) has (\d+) positions; at time=0, it is at position (\d+).")
    disks = []
...
def main():
    ...
    print(f"Waiting {find_delay(data)} seconds is the minimum wait that gets you a capsule")
    print()
