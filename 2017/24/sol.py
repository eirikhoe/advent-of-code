...
class Bridge:
    ...
    def _long_strong_step(self, end_connector, used):
            ...
            if i not in used:
                for j, connector in enumerate(component):
                    if connector == end_connector:
                        curr_used = used.copy()
                        curr_used.append(i)
                        bridge = sum(component) + self._long_strong_step(
                            component[1 - j], curr_used
                        )
        ...
        if strongest_bridge >= 0:
            ...
            if (len(new_used) > len(self.longest_ind)) or (
                (len(new_used) == len(self.longest_ind))
                and (strong_long > self.longest_strength)
            ):
