...
class State:
    def __init__(self, info):
        self.values = []
        for i in range(2):
            self.values.append({'write_val':int(info[3*i+0]),
...
class Turing:
    ...
    def __init__(self, data):
        ...
        for info in states_info:
            self.states[info[0]] = []
            for i in range(2):
                self.states[info[0]].append({'write_val':int(info[3*i+1]),
    ...
    def print_map(self):
        ...
        map_array[
            0, 2 * (self.loc - column_dim[0])
        ] = 2
        map_array[
            0, 2 * (self.loc - column_dim[0]) + 2
        ] = 3
        s = "\n".join(
            [
