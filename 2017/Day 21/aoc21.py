    ...
    def step(self):
        ...
        for i in np.arange(n_parts):
            for j in np.arange(n_parts):
                square = self.image[i*square_size:(i+1)*square_size,j*square_size:(j+1)*square_size]
                ...
                for k in np.arange(self.rules_start[square_size].shape[0]):
                    ...
                    if np.any(np.all(np.all(rule == rotations,axis=0),axis=0),axis=0):
                        new_image[i*new_square_size:(i+1)*new_square_size,j*new_square_size:(j+1)*new_square_size] = self.rules_end[square_size][k]
                        break
...
def main():
    ...
    print(f"{f.count_on_pixels()} pixels stay on after {n_iterations+additional_iterations} iterations")
