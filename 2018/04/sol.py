...
def main():
    ...
    messages = [
        [datetime.strptime(message[0], "%Y-%m-%d %H:%M"), message[1]]
        for message in find_coords.findall(data)
    ]
    ...
    for guard_id in guards:
        ...
        for shift in guards[guard_id]:
            sleep_grid = np.concatenate(
                [sleep_grid, guards[guard_id][shift].reshape((-1, 60))], axis=0
            )
        ...
        print(
            "\n".join(
                [
                    "".join([str(d) for d in row]).replace("0", ".").replace("1", "#")
    ...
    print(
        f"The sleepiest guard is #{sleepiest_guard_one} with {most_minutes_asleep} minutes slept"
    )
    print(f"He slept the most in minute {sleepiest_minute_one}")
    print(
        f"Thus the answer to Part 1 is {int(sleepiest_guard_one)*sleepiest_minute_one}"
    )
    print()
    ...
    print(
        f"Thus the answer to Part 2 is {int(sleepiest_guard_two)*sleepiest_minute_two}"
    )
