...
class Battle:
    ...
    def bfs(self, pos):
        ...
        while len(queue) > 0:
            ...
            for candidate in _get_candidates(point):
                if (distance[candidate] < 0) and (
                    self.map[candidate[0]][candidate[1]] != "#"
                ):
            ...
            if (1 not in enemy_distances) and (max(enemy_distances) > 0):
                ...
                for j in enemy_index:
                    for pos in _get_candidates(self.troops[j].pos):
                        if (self.map[pos[0]][pos[1]] == ".") and (
                            pos not in open_squares
                        ):
                ...
                for candidate in _get_candidates(self.troops[i].pos):
                    if (distance_from_dest[candidate] == min_dist - 1) and (
                        self.map[candidate[0]][candidate[1]] == "."
                    ):
                        new_pos = candidate
                        break
    ...
    def sim_battle(self):
        ...
        print(
            f"{winner_dict[self.troops[0].race]} win with {total_hp_left} total hit points left"
        )
