...
class Cave:
    ...
    def expand(self, new_lim):
        new_columns = np.zeros(
            (self.lims[0] + 1, new_lim[1] - self.lims[1]), dtype=np.int16
        )
    ...
    def shortest_route(self):
        ...
        while len(queue) > 0:
            ...
            if (point[0] >= self.lims[0]) or (point[1] >= self.lims[1]):
                self.expand(
                    (max(point[0], self.lims[0]) + 1, max(point[1], self.lims[1]) + 1)
                )
            for candidate in self._get_candidates(point):
                ...
                if (candidate not in time) or (t < time[candidate]):
                    ...
                    lim_t = (
                        time_bound
                        - abs(candidate[0] - self.target[0])
                        - abs(candidate[1] - self.target[1])
                        - 7 * int(candidate[2] != 1)
                    )
                    ...
                    if t < lim_t:
                        ...
                        if (candidate[:2] == self.target) and (
                            (t + 7 * int(candidate[2] != 1)) < time_bound
                        ):
...
def main():
    ...
    c = Cave(8103, (9, 758))
    print(
        f"The total risk for the smallest rectangle including the target and mouth is {c.find_risk_level(c.target)}"
    )
