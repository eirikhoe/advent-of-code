...
class Disease:
    ...
    def find_target(self, group):
        ...
        for target in range(lims[1] - lims[0]):
            ...
            if (self.targeted[target + lims[0]] == False) and (
                possible_damage[target] > 0
            ):
                possible_targets.append(target + lims[0])

        self.possible_targets[group] = possible_targets.copy()

        desireable_order = np.lexsort(
            (
                self.initiative[lims[0] : lims[1]],
                self.ep[lims[0] : lims[1]],
                possible_damage,
            )
        )[::-1]
    ...
    def battle(self, verbose=False):
        n_attackers = 1
        while (
            (self.n_defenders < len(self.n_units))
            and (self.n_defenders > 0)
            and (n_attackers > 0)
        ):
            if verbose:
                print("Immune System:")
                for group in range(self.n_defenders):
                    ...
                    print(
                        f"Group {self.group_ids[group]} contain {self.n_units[group]} unit{plural}"
                    )
                print("Infection:")
            ...
            if verbose:
                for group in range(self.n_defenders, len(self.n_units)):
                    for target in self.possible_targets[group]:
                        print(
                            f"Infection group {self.group_ids[group]} would deal defending group {self.group_ids[target]} {self.get_damage(group,target)} damage"
                        )
                for group in range(self.n_defenders):
                    for target in self.possible_targets[group]:
                        print(
                            f"Immune system group {self.group_ids[group]} would deal defending group {self.group_ids[target]} {self.get_damage(group,target)} damage"
                        )
                print()
            ...
            if verbose:
                for group in attackers:
                    ...
                    print(
                        f"{army} group {self.group_ids[group]} attacks defending group {self.group_ids[self.target[group]]}, killing {self.kills[group]} unit{plural}"
                    )
                print()
...
def main():
    ...
    print(
        f"With a boost of {upper} the Immune System army wins with {units_left} units left"
    )
