...
class Droid:
    ...
    def update_distance(self):
        ...
        for dir in range(1, 5):
            loc = self.new_location(dir)
            if (loc in Droid.distance) and (
                np.abs(Droid.distance[loc] - Droid.distance[self.location]) != 1
            ):
    ...
    def print_map(self):
        ...
        print(
            f"\033[2;1HDistance: {str(Droid.distance[self.location]).rjust(3,'0')}\t\t Max distance: {str(Droid.max_distance).rjust(3,'0')}"
        )
        print(
            "\n".join(
                [
    ...
    def find_move_dir(self):
        ...
        for i, possible_direction in enumerate(
            self.rotate(self.last_move_dir, [0, 90, 180, 270])
        ):
