    ...
    def get_intersections(self):
        ...
        for j in range(1,len(self.map)-1):
            for i in range(1,len(self.map[0])-1):
                if self.map[j][i] == self.map[j-1][i] == self.map[j+1][i] == self.map[j][i+1] == self.map[j][i-1] == ord('#'):
            ...
            if op_mode == 99:
                ...
                if op_mode == 4:
                    if self.prog.output < 128:
                        ...
                        if self.prog.output in [ord(char) for char in ['.','#','^','>','<','v','X']]:
                            if self.map[-1]:
...
def main(): 
    ...
    print(f"The sum of the alignment parameters is {camera.find_sum_alignment_parameters()}")
    print()
    print("Part 2")
    camera.wake_droid()
    camera.run()
