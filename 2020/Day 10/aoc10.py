...
def find_adapter_orderings(joltages):
    ...
    for i, _ in enumerate(splits[:-1]):
        group = joltages[splits[i] : splits[i + 1]]
        amount *= _group_orderings(group[1:], group[0], group[-1])

    return amount


def _group_orderings(joltages, current, end):
    if current == end:
        return 1
    amount = 0
    for i, joltage in enumerate(joltages):
        if joltage - current > 3:
            break
        amount += _group_orderings(joltages[i + 1 :], joltage, end)
    return amount
